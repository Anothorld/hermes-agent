...
- **Login state** — with `chrome-devtools-mcp` connected to a real Chrome via CDP (`--browserUrl http://127.0.0.1:9222`), you inherit whatever sessions that profile is logged into. KOL skills use **local debug Chrome** via built-in `browser_*` tools (same stateful model). Skills should assume "stateful, possibly-already-logged-in." Login walls become a legitimate code path: take_snapshot → check for login form → bail out and ask the user to log in once in the debug Chrome.
