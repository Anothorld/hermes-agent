...
- **Conversation context** (`CS_INTENT_CONTEXT_TURNS`, default 3) — the seam extracts recent prior messages from the same thread (visitor + operator, excluding system/bot/internal-note) and passes them as `conversation_history` to the classifier. This lets the LLM understand reply context (e.g., agent said "order ships July 10" → customer replies "ok but change address" → intent is `order_management`, not spam). Quoted reply content (`On ... wrote:`, `-----Original Message-----`, `>` lines, forwarded blocks) is stripped from each history message. The classified email is always the latest visitor message; history is context only. Set `CS_INTENT_CONTEXT_TURNS=1` to disable.
