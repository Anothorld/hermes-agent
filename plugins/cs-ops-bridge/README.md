...
- **Rearm follow-up scan** (`run_rearm_scan_once`): scans `operator_replied` / `reviewed` for newer QuickCEP visitor mail and resets to `pending` when SIO/`--unread-only` missed the follow-up. Matching must **not** rely on substring id alone — REST stores `rest:{lastMsgTime}` while `messages` returns a native id. `is_newer_visitor_followup` requires either an id match **or** (for REST markers) `createTime` **strictly newer** than the recorded lastMsgTime; equal/older createTime (or missing createTime) does not re-arm. Prevents false `reviewed`→`pending` that shows Console「处理中 / 草稿缺失」after结案.
