...
| `CS_INTENT_ENABLED` | `false` | Delegate intent gate to `cs-intent-classifier` |
| `CS_INTENT_BASE_URL` | `http://127.0.0.1:8082` | Classifier HTTP base |
| `CS_INTENT_SEAM_TIMEOUT` | `150` | Hard cap (seconds) for pre-fetch + `POST /classify` (`future.result` + urlopen). Must be ≥ `CS_INTENT_LLM_TIMEOUT` |
