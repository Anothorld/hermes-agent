...
5. **[REQUIRED] Record Q&A to Hindsight Knowledge bank** BEFORE draft-save, via the `knowledge_retain` MCP tool. This step is NOT optional — skipping it means future identical questions will be re-escalated:
   ...
   If the MCP server is unavailable, retry via terminal with the direct API curl (items + 300s timeout). The curl is a LAST-RESORT fallback — it bypasses the refine pipeline, so you MUST manually de-identify (remove order#/email/phone/tracking) and pick the right domain tags before sending. Use ONE of these two shapes (not both):
   - Product fact (SKU known): tags=["product:<SKU>","category:<assembly|materials|color|...>"], entities=[{{"text":"product:<SKU>","type":"product"}},{{"text":"<key concept>","type":"CONCEPT"}}], metadata={{"domain":"product","product_id":"<SKU>","source":"human_confirmed","confirmed":"true","env":"{env}"}}
   - Policy fact (no SKU): tags=["policy:<return|warranty|shipping|installation|payment|swatch>","category:<return-policy|warranty|shipping-policy|swatch-sample|...>"], entities=[{{"text":"policy_type:<type>","type":"policy_type"}},{{"text":"applies_to:all","type":"applies_to"}},{{"text":"<key concept>","type":"CONCEPT"}}], metadata={{"domain":"policy","policy_type":"<type>","applies_to":"all","source":"human_confirmed","confirmed":"true","env":"{env}"}}
   Full curl (replace <...>):
   terminal: curl -s -m 300 -X POST {knowledge_url}/v1/default/banks/{knowledge_bank}/memories -H 'Content-Type: application/json' -d '{{"async":true,"items":[{{"content":"<de-identified Q&A>","context":"povison product knowledge" or "povison policy knowledge","tags":["<product:<SKU>> or <policy:<type>>","category:<cat>"],"timestamp":"unset","observation_scopes":"shared","entities":[<fill per shape above>],"metadata":{{<fill per shape above>}}}}]}}'
   NEVER use "escalation-qa" or "session:*" as tags — they fragment observations. If the fact has both a SKU and a policy, send TWO curl calls (one product, one policy) rather than mashing them.
