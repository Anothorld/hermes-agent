...
- **cover_reels**: first N reels from the profile grid (default 2) for
  ``vision_analyze`` / cover download.
- **video_reels**: when video eval is ON, a random sample of M reels (default 3)
  from the recent ``EVAL_REELS_POOL`` (default 10) — not top-by-views.
...
def build_content_eval_plan(
    ...
    *,
    ...
    video_pool_size: int = EVAL_REELS_POOL,
    handle: str = "",
    seed: int | None = None,
) -> dict[str, Any]:
    ...
    Args:
        ...
        eval_mode: ``"cover"`` or ``"video"``. Resolved from env/brief when
            omitted.
        cover_count: How many recent reels to use for cover / vision screening.
        video_count: How many reels to randomly sample for video deep-eval
            when ``eval_mode == "video"``.
        video_pool_size: Recent-grid window to sample videos from (independent
            of ``cover_count`` so covers can stay small while video samples
            from a wider recent set).
    ...
    cover_reels = [
        _slim_reel(r) for r in reels[:cover_count] if r.get("url")
    ]

    video_reels: list[dict[str, Any]] = []
    selection_note = "covers_only"
    video_pool = list(reels[:video_pool_size])
    if mode == "video" and video_pool:
        rng = random.Random(seed if seed is not None else _seed_for_handle(handle))
        k = min(video_count, len(video_pool))
        if k > 0:
            picked = rng.sample(video_pool, k)
            video_reels = [_slim_reel(r) for r in picked]
            selection_note = f"random_{k}_from_recent_{len(video_pool)}"
    ...
    return {
        ...
        "selection": {
            ...
            "pool_size": len(video_pool) if mode == "video" else len(cover_reels),
            "cover_count": len(cover_reels),
        },
    }
