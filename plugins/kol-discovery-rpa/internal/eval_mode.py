...
When OFF (default):
    2 covers + 10 comments → vision_analyze(cover) + comments
When ON:
    2 covers + 3 random videos (from recent 10) + 10 comments → above + video_analyze(video_reels)
...
# Covers downloaded / vision_analyze'd per candidate (cost control).
EVAL_COVER_COUNT = 2
# Random video sample size when video eval is ON.
EVAL_VIDEO_COUNT = 3
# Recent-grid pool size for video sampling and comment ER collection.
EVAL_REELS_POOL = 10
