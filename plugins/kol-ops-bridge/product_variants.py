"""Product URL -> variant parser (Povison ``modelProductList`` API + query fallback).

Mirrors Console ``app/product_variants.py`` so bridge contract rendering can
resolve ``merchant_sku`` / color / size without a pre-seeded catalog row.
"""
...
def parse_variants_from_url(url: str) -> list[dict[str, Any]]:
    """Return variant descriptors parsed from ``url`` (Povison API or query string)."""
...
__all__ = ["parse_variants_from_url"]
