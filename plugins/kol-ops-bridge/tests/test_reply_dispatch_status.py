...
def test_reply_dispatch_status_skip_when_handled(bridge_pkg, cal_db):
    cal = cal_db
    iid = cal.upsert_identity(primary_handle="@t4", primary_email="t4@example.com", env="LIVE")
    cid = "C4"
    cal.upsert_campaign_config(campaign_id=cid, env="LIVE")
    mid = "msg-handled"
    cal.write_event(
    ...
    assert out["has_reply_handled_event"] is True
    assert out["should_skip_poller"] is True
    assert out["should_retry_gateway_only"] is False


def test_reply_dispatch_status_skip_when_dispatch_exhausted(bridge_pkg, cal_db):
    cal = cal_db
    iid = cal.upsert_identity(primary_handle="@t5", primary_email="t5@example.com", env="LIVE")
    cid = "C5"
    cal.upsert_campaign_config(campaign_id=cid, env="LIVE")
    mid = "msg-exhausted"
    cal.write_event(
    ...
    cal.write_event(
        ...
        payload={"message_id": mid, "retry_cap": 8},
        env="LIVE",
    )
    out = cal.reply_dispatch_status(
        identity_id=iid, campaign_id=cid, message_id=mid, env="LIVE",
    )
    assert out["has_dispatch_exhausted_event"] is True
    assert out["should_skip_poller"] is True
    assert out["should_retry_gateway_only"] is False
