...
class TestNoBundledSkillsOptOut:
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.get_hermes_home", return_value=hermes_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.get_hermes_home", return_value=home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync._skills_dir", return_value=skills_dir), \
             patch("tools.skills_sync._manifest_file", return_value=manifest_file), \
             patch("tools.skills_sync.get_hermes_home", return_value=home):
            sync_skills(quiet=True)
            # User edits 'beta'
