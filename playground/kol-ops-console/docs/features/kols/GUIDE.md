...
| 活动批准 `approve-shortlist` | Console 对每个缺邮箱的已批 identity 自动排队 `kol-email-discover:*`（Gate B 同步 + browser）；**不再**对同一 identity 单独排 brief（Step 6 负责）；缺 brief 且有邮箱的 identity 排 `kol-creator-brief-refresh:*` |
| 活动审批 run（`kol-campaign-outreach:*`） | 仅对已有 `primary_email` 且非 `pending_creator_brief` 的身份起草；`# email_discovery_queued` / `# creator_brief_queued` 中的身份跳过 |
| `kol-email-discover:*` 完成（approve 排队） | reconciler：`primary_email` 已写入 **且 creator brief ready** → auto-draft；brief 仍缺 → 补排队 `kol-creator-brief-refresh`（audit `kol.creator_brief.refresh_after_email_discover`） |
| `kol-creator-brief-refresh:*` 完成 | reconciler：brief ready + 有邮箱 → auto-draft（audit `campaign.auto_draft_after_creator_brief_refresh`） |
