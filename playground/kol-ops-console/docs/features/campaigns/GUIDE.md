...
| **brief 上下文保护** | rediscover brief 只注入 **最近 5 轮** `prior_runs`（每轮列表字段最多 15 条）、`already_discovered` **抽样 25 + count**；bootstrap CLI 用 `--summary`。避免成熟活动（100+ 轮）把 100KB+ 历史塞进 prompt 导致 LLM 流挂死 |
| **premature bootstrap stop** | 终态无真实探索（空 `visited_handles`/`attempted_angles`/`floor_unmet_reason`，或 soft-stop 文案）→ 打标 `exit_kind=premature_bootstrap_stop`，强制带 `# premature_exit_recovery` 的 auto-retry；**不计入** niche `consecutive_zero` streak。超过 2 次 premature 或触达 `MAX_AUTO_RETRIES` → escalate `premature_exit_repeated`（模型早停，非 niche 枯竭）。gateway 驱逐且 `run_info=None` 时不打 premature 标，沿用原 zero-new/auto-retry |
