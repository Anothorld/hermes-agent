# KOL Ops Console — 功能指引索引

先读总览：[../ARCHITECTURE.md](../ARCHITECTURE.md)

每个 **系统功能** 一份 `GUIDE.md`：业务说明、操作员路径、前后端文件对照、主要 API、关联模块。

| 目录 | 说明 |
|------|------|
| [features/auth-settings/GUIDE.md](features/auth-settings/GUIDE.md) | 登录、用户、设置、审计 |
| [features/products/GUIDE.md](features/products/GUIDE.md) | SKU 产品目录 |
| [features/campaigns/GUIDE.md](features/campaigns/GUIDE.md) | 活动生命周期、候选人池、回复监听 |
| [features/kols/GUIDE.md](features/kols/GUIDE.md) | KOL 看板、详情、事实、目标、关系 |
| [features/approvals/GUIDE.md](features/approvals/GUIDE.md) | 待审批队列、回信草稿、结构化驳回 |
| [features/learning/GUIDE.md](features/learning/GUIDE.md) | 自主学习、策略反哺 |
| [features/escalations/GUIDE.md](features/escalations/GUIDE.md) | 升级工单 |
| [features/policies/GUIDE.md](features/policies/GUIDE.md) | 策略 Markdown 编辑 |
| [features/gate-metrics/GUIDE.md](features/gate-metrics/GUIDE.md) | 门禁效果看板 |
| [features/agent-gateway/GUIDE.md](features/agent-gateway/GUIDE.md) | Agent 运行、SSE 日志、Gateway 审批 dock |
| [features/gmail/GUIDE.md](features/gmail/GUIDE.md) | 操作员 Gmail OAuth、邮件历史 |
| [features/nox/GUIDE.md](features/nox/GUIDE.md) | Nox 补充、尽调、配额 |
| [features/live-events/GUIDE.md](features/live-events/GUIDE.md) | WebSocket 实时刷新 |
| [features/performance/GUIDE.md](features/performance/GUIDE.md) | Agent 爆发期性能、压测、运维 runbook |

**代码侧速查**

- 后端 API：`../backend/app/routers/README.md`
- 前端页面：`../frontend/src/pages/README.md`
- 前端组件簇：`../frontend/src/components/README.md`
