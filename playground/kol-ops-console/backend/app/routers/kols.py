...
_DISCOVER_EMAIL_INSTRUCTIONS = (
    ...
    "   **Tool names are EXACT:** Tier 1 = `web_search` (search) +\n"
    "   `web_extract` (fetch+read a URL). There is NO `WebSearch`/`WebFetch`/\n"
    "   `GoogleSearch` and no CLI `web-search` subcommand. Tier 2 (JS-gated:\n"
    "   Instagram / Linktree / Beacons) = call `browser_navigate` then\n"
    "   `browser_snapshot` directly. If you 'can't find' a search/fetch tool,\n"
    "   it is `web_search`/`web_extract` — never substitute terminal HTTP.\n"
