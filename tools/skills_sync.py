...
def sync_skills(quiet: bool = False) -> dict:
    ...
    if (get_hermes_home() / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = get_hermes_home() / NO_BUNDLED_SKILLS_MARKER
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (get_hermes_home() / NO_BUNDLED_SKILLS_MARKER).exists()
